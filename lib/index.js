...
const ResponseNotDefinedError = errors.ResponseNotDefinedError;
...
function createRouter(spec, options) {
  function getControllerMethod(controllerDirectory, controllerName, controllerMethodName, strictMode, method, route) {
    ...
    /* Fallback */
    return function* () {
      throw new RouteNotImplementedError(method, route);
    }
  }
    ...
    _.forOwn(routes, function (methods, route) {
      ...
      _.forOwn(methods, function (methodInfo, method) {
        ...
        if (router[method]) {
          ...
          const responseValidator = createResponseValidator(method, route, methodInfo.responses);
          router[method](route, requestValidator, responseValidator, controllerMethod);
        } else {
...
function createRequestValidator(method, route, parameterDefinitions) {
    ...
    try {
      ...
      if (parameterDefinition.in === 'body') {
        if (!parameterDefinition.schema) {
          throw new TypeValidationError(parameterDefinition.name, parameterDefinition.type, parameterDefinition.format, parameterValue, `Schema not found.`);
        }
  ...
  return function* validate(next) {
    try {
      ...
      if (err instanceof RouteNotImplementedError) {
...
function createResponseValidator(method, route, responseDefinitions) {
  function validateHeaders(ctx, responseDefinition) {

  }

  function validateBody(ctx, responseDefinition) {
    const body = ctx.body;
    if (responseDefinition.schema) {
      validateSchema('TODO', 'TODO', 'TODO', responseDefinition.schema, body);
    }
  }

  function validateResponse(ctx, responseDefinition) {
    validateHeaders(ctx, responseDefinition);
    validateBody(ctx, responseDefinition);
  }

  function getResponseDefinition(ctx, responseDefinitions) {
    const responseDefinition = responseDefinitions[ctx.status];
    if (!responseDefinition) {
      throw new ResponseNotDefinedError(method, route);
    }
  ...
  return function* validate(next) {
    ...
    try {
      const responseDefinition = getResponseDefinition(this, responseDefinitions);

      validateResponse(this, responseDefinition);
    } catch (err) {
      debug(err);

      if (err instanceof ResponseNotDefinedError) {
        this.status = HTTPStatus.INTERNAL_SERVER_ERROR;
        this.body = {
          code      : 9001, // TODO
          message   : 'No bueno', // TODO
          route     : {
            method : method,
            path   : route
          },
          parameter : {
            expected : _.keys(responseDefinitions),
            actual   : this.status
          }
        };
      } else {
