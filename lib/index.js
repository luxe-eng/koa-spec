...
function createRequestValidator(method, route, parameterDefinitions) {
  ...
  function validateProperties(name, type, format, properties, value) {
    ...
    _.forOwn(properties, (propertyInfo, propertyName) => {
      const validatedPropertyValue = validateValue(propertyName, propertyInfo.type, propertyInfo.format, propertyInfo.items, propertyInfo.properties, value[propertyName]);
      actualValue[propertyName] = validatedPropertyValue;
    });
    return actualValue;
  }

  function validateValue(name, type, format, items, properties, value) {
    switch (type) {
      case 'boolean':
      ...
      case 'object':
        return validateProperties(name, type, format, properties, value);
      case 'array':
  ...
  function validateParameter(parameterDefinition, parameterValue) {
    try {
      ...
      if (parameterDefinition.in === 'body') {
        ...
        return validateValue(parameterDefinition.name, parameterDefinition.type, parameterDefinition.format, parameterDefinition.items, undefined, parameterValue);
      }
    } catch (err) {
