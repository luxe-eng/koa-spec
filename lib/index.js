    ...
    switch (format) {
      case 'int32':
        if (actualValue > INTEGER_MAX_VALUE) {
          throw new FormatValidationError(name, type, format, value, `Maximum: ${INTEGER_MAX_VALUE}.`);
        } else if (actualValue < INTEGER_MIN_VALUE) {
          throw new FormatValidationError(name, type, format, value, `Minimum: ${INTEGER_MIN_VALUE}.`);
        }
        break;
      case 'int64':
        if (actualValue > Number.MAX_SAFE_INTEGER) {
          throw new FormatValidationError(name, type, format, value, `Maximum integer value: ${Number.MAX_SAFE_INTEGER}.`);
        } else if (actualValue < Number.MIN_SAFE_INTEGER) {
          throw new FormatValidationError(name, type, format, value, `Minimum integer value: ${Number.MIN_SAFE_INTEGER}.`);
        }
        break;
      default:
    ...
    switch (format) {
      case 'float':
        if (actualValue > FLOAT_MAX_VALUE) {
          throw new FormatValidationError(name, type, format, value, `Maximum float value: ${FLOAT_MAX_VALUE}.`);
        } else if (actualValue < FLOAT_MIN_VALUE) {
          throw new FormatValidationError(name, type, format, value, `Minimum float value: ${FLOAT_MIN_VALUE}.`);
        }
        break;
      case 'double':
        break;
